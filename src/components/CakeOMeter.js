...
import { tinScalingFactor, scaleIngredients } from '../logic/convertIngredients';
  ...
  return <div id="container">
    <div id="cakeometer-inputs">
      ...
      {ingredients.map((ingredient, index) =>
        <IngredientEditor key={index} ingredient={ingredient} index={index}
          onUpdate={updateIngredient} onRemove={removeIngredient}
        />
      )}
      ...
      {(convertedIngredients === null ?
        ...
        : <table className="ingredient-list">
          ...
          <tbody>{convertedIngredients.map((ingredient, index) => <tr key={index}>
            ...
            <td>{ingredient.quantity.toFixed(1)} {ingredient.unit}</td>
          </tr>)}</tbody>
        </table>
      )}
    </div>
